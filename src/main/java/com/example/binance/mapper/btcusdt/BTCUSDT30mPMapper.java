...
@Repository
public interface BTCUSDT30mPMapper extends KlineBaseMapper {

    /**
     * 根据开盘时间查询单条K线
     */
    @Select("SELECT * FROM btcusdt_perpetual_30m_kline WHERE open_time = #{openTime}")
    KlineEntity selectByOpenTime(@Param("openTime") Long openTime);

    /**
     * 根据开盘时间删除K线数据
     */
    @Delete("DELETE FROM btcusdt_perpetual_30m_kline WHERE open_time = #{openTime}")
    int deleteByOpenTime(@Param("openTime") Long openTime);

    /**
     * 查询所有K线数据
     */
    @Select("SELECT * FROM btcusdt_perpetual_30m_kline ORDER BY open_time ASC")
    List<KlineEntity> selectAll();

    /**
     * 查询指定时间范围的K线数据
     */
    @Select("SELECT * FROM btcusdt_perpetual_30m_kline WHERE open_time >= #{startTime} AND open_time <= #{endTime} ORDER BY open_time ASC")
    List<KlineEntity> selectByTimeRange(@Param("startTime") Long startTime, @Param("endTime") Long endTime);

    @Insert("""
            <script>
                INSERT INTO btcusdt_perpetual_30m_kline (
                    open_time,
